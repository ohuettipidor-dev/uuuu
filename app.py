...
def render_mentions(text, current_user_id=None):
    ...
    for user in users:
        ...
        if hasattr(user, 'username_link') and user.username_link:
...
# ========== ЧАТ - ОСНОВНОЙ МАРШРУТ С ВСЕМИ ЧАТАМИ ==========
@app.route('/chat')
@login_required
def chat():
    current_user.last_seen = datetime.utcnow()
    ...
    for u in users:
        ...
        convs.append({
            ...
            'id': u.id, 
            'name': u.username, 
            'username_link': u.username_link if hasattr(u, 'username_link') else None,
            'avatar': u.avatar, 
            'status': u.status, 
            'last': last, 
            'unread': unread
...
# ========== ВСПОМОГАТЕЛЬНЫЕ МАРШРУТЫ ДЛЯ ЧАТА ==========

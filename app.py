...
def channel_create():
    if request.method == 'POST':
        ...
        if username and Channel.query.filter_by(username=username).first():
            ...
            return redirect(url_for('channel_create'))
        channel = Channel(name=name, username=username, description=description, created_by=current_user.id)
...
@login_required
def premium_page():
    if get_premium_status(current_user.id):
        return render_template('premium_active.html')

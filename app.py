...
from blockchain import is_ready, transfer_onchain, init_blockchain
import os
import uuid
import re
import json
...
def chat():
    ...
    for g in groups:
        ...
        unread = GroupMessage.query.filter_by(group_id=g.id).filter(GroupMessage.sender_id != current_user.id).count()
        convs.append({
...
@login_required
def withdraw():
    # Если пришло поле ton_address – это вывод GRRR (твой рабочий код)
    if request.form.get('ton_address'):
    ...
    # Иначе это вывод рублей на ЮMoney (восстановленная логика)
